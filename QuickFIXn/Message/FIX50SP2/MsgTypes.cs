...
namespace QuickFix
{
    namespace FIX50SP2
    {
        internal static class MsgTypes
        {
            ...
            public const int SettlementObligationReport = 'B' + ('Q' << 8);
            public const int DerivativeSecurityListUpdateReport = 'B' + ('R' << 8);
            public const int TradingSessionListUpdateReport = 'B' + ('S' << 8);
            public const int MarketDefinitionRequest = 'B' + ('T' << 8);
            public const int MarketDefinition = 'B' + ('U' << 8);
            public const int MarketDefinitionUpdateReport = 'B' + ('V' << 8);
            public const int ApplicationMessageRequest = 'B' + ('W' << 8);
            public const int ApplicationMessageRequestAck = 'B' + ('X' << 8);
            public const int ApplicationMessageReport = 'B' + ('Y' << 8);
            public const int OrderMassActionReport = 'B' + ('Z' << 8);
            public const int OrderMassActionRequest = 'C' + ('A' << 8);
            public const int UserNotification = 'C' + ('B' << 8);
            public const int StreamAssignmentRequest = 'C' + ('C' << 8);
            public const int StreamAssignmentReport = 'C' + ('D' << 8);
            public const int StreamAssignmentReportACK = 'C' + ('E' << 8);
            public const int PartyDetailsListRequest = 'C' + ('F' << 8);
            public const int PartyDetailsListReport = 'C' + ('G' << 8);
        }
