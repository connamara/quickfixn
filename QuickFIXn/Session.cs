    ...
    public class Session : IDisposable
    {
        ...
        protected void NextLogout(Message logout)
        {
            ...
            else
            {
                ...
                if(ResetOnLogout)
                    _state.Reset("ResetOnLogout");
                else if (CmeMode && logout.IsSetField(789) && logout.GetInt(789) == 1) {
        ...
        private bool _didHandleCmeModeGapFillRun = false;

        protected void NextSequenceReset(Message sequenceReset)
        {
            _didHandleCmeModeGapFillRun = false;
            ...
            // This may possibly set _didHandleCmeModeGapFillRun=true if in CmeMode
            if (!Verify(sequenceReset, isGapFill, isGapFill))
                return;

            if (_didHandleCmeModeGapFillRun)
            {
                _didHandleCmeModeGapFillRun = false;
        ...
        private void HandleCmeModeGapFill(Message seqReset)
        {
            ...
            _didHandleCmeModeGapFillRun = true;
        }
            ...
            try
            {
                ...
                if (checkTooHigh || checkTooLow)
                {
                    ...
                    if (IsResendRequested)
                    {
                        ...
                        if (CmeMode && msgType == MsgType.SEQUENCE_RESET && msg.GetBoolean(Tags.GapFillFlag))
                        {
                            HandleCmeModeGapFill(msg);
                        }
        ...
        protected void DoTargetTooHigh(Message msg, SeqNumType msgSeqNum)
        {
            ...
            if (IsResendRequested)
            {
                ...
                if (CmeMode && !range.IsResendStarted)
                {
