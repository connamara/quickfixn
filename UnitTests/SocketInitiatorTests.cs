...
namespace UnitTests
{
    ...
    class SocketInitiatorTests
    {
            ...
            foreach (Match m in Regex.Matches(msgText, FIXMessageDelimit))
            {
                ...
                if (Regex.IsMatch(socketState._messageFragment, FIXMessageEnd))
                {
                    ...
                    if (message.Header.GetString(QuickFix.Fields.Tags.MsgType) == QuickFix.Fields.MsgType.LOGON)
                    {
                        ...
                        if (_disconnectOnLogon)
                        {
                            socketState._socket.Disconnect(true);
                        }
        ...
        [Test]
        public void Disconnecting_An_Initiator_At_End_Of_Session_Should_Allow_It_To_Reconnect_At_Start()
        {
            // GIVEN - a listening socket that will disconnect on receiving a logon message from an initiator
            _disconnectOnLogon = true;
            StartListener();
            StartEngine(DateTime.UtcNow.AddSeconds(2).ToString("hh:mm:ss"),DateTime.UtcNow.AddSeconds(1).ToString("hh:mm:ss"));
            ...
            Assert.False(_sessions[StaticInitiatorCompID]._socket.Connected);
            _sessions.Remove(StaticInitiatorCompID);
            
            //AT Session Start Time the initiator should be able to logon again 
